"""توليد موسيقى أجمكان — مقطوعة أصلية بطول الريلز بالضبط.

لماذا التوليد بدل التنزيل: الموسيقى التجارية بلا ترخيص تُعرّض الفيديو للكتم
أو الحذف على تيك توك. وهذه مُولَّدة هنا بالكامل، فلا حقوق على أحد.

ولماذا بطول الريلز بالضبط: لو وُلِّدت أطول ثم قصّها ffmpeg عند نهاية الفيديو
لضاع التلاشي الأخير وبدا الصوت مبتورًا. لذلك يُمرَّر طول الخط الزمني الفعلي
فيبدأ التلاشي وينتهي مع آخر إطار.

الشكل: وسادة هارمونية + أربيجيو ناعم + نبض اختياري، على ستة أمزجة تختلف في
السلّم والإيقاع والترشيح والصدى.
"""
...
def build(seconds: float, mood: str) -> np.ndarray:
    ...
    # دخول وخروج ناعمان — يتناسبان مع الطول فلا يبتلعان مقطعًا قصيرًا
    fade = int(min(1.6, seconds * 0.10) * SR)
...
def write_track(path: str | Path, seconds: float, mood: str) -> Path:
    """يكتب المقطع بصيغة AAC جاهزة للدمج في الفيديو."""
    audio = build(seconds, mood)
